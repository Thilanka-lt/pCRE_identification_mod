...
def main():
	...
	req_group.add_argument('-path_p', help='path of the pipeline', required=True)
	req_group.add_argument('-path_k', help='path of the kmer lists', required=True)
	...
	for train_fold_index, val_fold_index in cv.split(train,train):
		...
		for kmer in args.kmers.split(','):
			...
			os.system('python %s -f make_df -k %s -p FastaFiles/train_pos_geneID_%s_cv_%s.txt.fa -n FastaFiles/train_neg_geneID_%s_cv_%s.txt.fa'%(os.path.join(args.path_p,"Pairwise_kmers.py"), os.path.join(args.path_k,kmer), number_training_pos,n, number_training_neg,n))
			k = int(kmer.split('mer')[0])
			if 'pair' not in kmer:
				os.system('python %s -f parse_df -df train_pos_geneID_%s_cv_%s_k%s_%s.0single_rc_df.txt -pval %s'%(os.path.join(args.path_p,"Pairwise_kmers.py"),number_training_pos,n, int(4**k/2),k, args.pval))
			else:
				os.system('python %s -f parse_df -df train_pos_geneID_%s_cv_%s_k%s_%s.0paired_rc_df.txt -pval %s'%(os.path.join(args.path_p,"Pairwise_kmers.py"),number_training_pos, n,int((4**k/2) * (4**k/2 -1)/2),k,args.pval))
	...
	# train
	os.system('python %s -f make_df -k Enriched_kmers.txt -p FastaFiles/train_pos_geneID_%s.txt.fa -n FastaFiles/train_neg_geneID_%s.txt.fa'%(os.path.join(args.path_p,"Pairwise_kmers.py"),number_training_pos, number_training_neg))
	os.system('mv train_pos* train_enriched_kmers_matrix.txt')
	# test
	os.system('python %s -f make_df -k Enriched_kmers.txt -p FastaFiles/test_pos_geneID_%s.txt.fa -n FastaFiles/test_neg_geneID_%s.txt.fa'%(os.path.join(args.path_p,"Pairwise_kmers.py"),number_holdout_pos, number_holdout_neg))
	os.system('mv test_pos* test_enriched_kmers_matrix.txt')
	
	# RandomForestClassifer
	print('Begin to train the model')
	os.system('python %s -file train_enriched_kmers_matrix.txt -cv_num %s -score %s -smote %s'%(os.path.join(args.path_p,"13_RF_holdout_before_enrichment_draw_two_AUC_SMOTE_upsampling_val_02_feature_selection.py"),args.cv_num, args.score,args.smote))
	...
	os.system('python %s -model train_enriched_kmers_matrix.txt_RF_cv%s_score_%s_%s.pkl -new_file test_enriched_kmers_matrix.txt -threshold %s -save_file train_enriched_kmers_applied_on_test_threshold -smote %s'%(os.path.join(args.path_p,"09_apply_model_to_new_data_draw_two_AUC_SMOTE_on_test_prediction_score.py"),args.cv_num, args.score,args.smote,threshold,args.smote_test))
